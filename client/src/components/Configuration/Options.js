...
export default function Options() {
  ...
  const handleToggle = value => () => {
    ...
    if (value === "notification" && newChecked.includes(value)) {
      console.log('notification 옵션 해제', value, 'index: ', newChecked.indexOf(value))
    ...
    // notification 옵션 활성하였을때 : 모두 활성화
      ...
      console.log('notification 옵션 활성', value, 'index: ', newChecked.indexOf(value))
      newChecked.push(value);
      newChecked.push("project", "chat");
    }
    // 나머지 옵션 활성 핸들러
    else if (currentIndex === -1 && !newChecked.includes(value)) {
      console.log('나머지 옵션 활성', value, 'index: ', newChecked.indexOf(value))
      newChecked.push(value);
      ...
      console.log('나머지 옵션 헤제', value, 'index: ', newChecked.indexOf(value))
  ...
  return (
    <Card>
      {/* [ Theme ] : 태마 설정 리스트 */}
      <List subheader={<ListSubheader>Theme</ListSubheader>} className={classes.root}>
      ...
      {/* [ Language ] : 언어 설정 리스트 */}
      <List subheader={<ListSubheader>Language</ListSubheader>} className={classes.root}>
        <ListItem>
          <ListItemIcon>
            <LanguageIcon />
          </ListItemIcon>
          <ListItemText id="switch-list-label-lang" primary="Korean / English" />
          <ListItemSecondaryAction>
            <Switch
              edge="end"
      ...
      {/* [ Alert ] : 전체 알림 설정 대분류 */}
      <List subheader={<ListSubheader>Alert</ListSubheader>} className={classes.root}>
        <ListItem>
          <ListItemIcon>
            <NotificationsIcon />
          </ListItemIcon>
          <ListItemText id="switch-list-label-notification" primary="Notification" />
          <ListItemSecondaryAction>
            <Switch
              edge="end"
        ...
        <List className={classes.notification}>
          <ListItem>
            ...
            <ListItemText id="switch-list-label-projectAlert" primary="Project notifications" />
            <ListItemSecondaryAction>
              <Switch
                edge="end"
                onChange={handleToggle('project')}
                checked={checked.indexOf('project') !== -1}
                inputProps={{ 'aria-labelledby': 'switch-list-label-projectAlert' }}
              />
            </ListItemSecondaryAction>
          </ListItem>
          <ListItem>
            ...
            <ListItemText id="switch-list-label-chatAlert" primary="Chat notifications" />
            <ListItemSecondaryAction>
              <Switch
                edge="end"
                onChange={handleToggle('chat')}
                checked={checked.indexOf('chat') !== -1}
                inputProps={{ 'aria-labelledby': 'switch-list-label-chatAlert' }}
              />
            </ListItemSecondaryAction>
          </ListItem>
